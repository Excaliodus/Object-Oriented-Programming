...
public:
	...
	int counter = 0;
	void ReadPosts(ifstream& postin)
	{
		char temp[80];
		postin >> posttype;
		postin >> temp;
		postID = Helper::GetStringFromBuffer(temp);
		uploaded.ReadDate(postin);
		if (counter == 0)
		{
			postin.ignore();
		}
		postin.getline(temp, 80);
		postcontent = Helper::GetStringFromBuffer(temp);
		if (posttype == 2)
		{
			activity = new Activity;
			activity->readActivity(postin);
		}
		counter++;
	}
	bool MemCheck()
...
public:
	...
	void viewHome()
	{
		cout << "----------------------------------------------------------------------------------------------------------\n" << firstname << " " << lastname << "\t-- Home Page" << endl << endl;
	...
	void viewtimeline()
	{
		cout << "----------------------------------------------------------------------------------------------------------\n" << firstname << " " << lastname << "\t-- TimeLine" << endl << endl;
		for (int i = 0; i < Tposts - 1; i++)
		{
			Timeline[i]->printpostmem();
		}
	}
...
public:
	...
	{
		ifstream pagein("Pages.txt");
		if (pagein.is_open())
		{
			pagein >> Pages::totalpages;
			pages = new Pages * [Pages::totalpages];
			for (int i = 0; i < Pages::totalpages; i++)
			{
				pages[i] = new Pages;
				pages[i]->ReadPages(pagein);
			}
			pagein.close();
		}
		else
	...
	void LoadPosts(const char* postfilename)
	{
		ifstream postin("Posts.txt");
		if (postin.is_open())
		{
			Object* objptr = nullptr;
			char temp[80];
			postin >> Post::Tposts;
			posts = new Post * [Post::Tposts];
			for (int i = 0; i < Post::Tposts; i++)
			{
				posts[i] = new Post;
				posts[i]->ReadPosts(postin);
				postin >> temp; 
				if (temp[0] == 'u')
				{
					objptr = SearchUser(Helper::GetStringFromBuffer(temp));
				}
				else if (temp[0] == 'p')
				{
					objptr = SearchPage(Helper::GetStringFromBuffer(temp));
				}
				posts[i]->sharedby_setter(objptr);
		...
		{
			cout << "Posts.txt has not opened" << endl;
		}
	}
	void LoadComments(const char* commentfilenaame)
	{
		ifstream commentin("comments.txt");
		if (commentin.is_open())
		{
			commentin >> Comments::Tcmnt;
			char temp[80];
			Object* objptr = nullptr;
			comments = new Comments * [Comments::Tcmnt];
			for (int i = 0; i < Comments::Tcmnt; i++)
			{
				comments[i] = new Comments;
				comments[i]->ReadCommments(commentin);
				commentin >> temp;
				Post* ptr = SearchPost(posts, Helper::GetStringFromBuffer(temp));
				commentin >> temp;
				if (temp[0] == 'u')
				{
					objptr = SearchUser(Helper::GetStringFromBuffer(temp));
				...
				commentin.getline(temp, 80);
				comments[i]->Content_setter(temp);
			}
			delete[] objptr;
			commentin.close();
		}
		...
		{
			cout << "comments.txt has not opened" << endl;
		}
	}
	...
	Pages* SearchPage(const char* page_id)
	{
		...
		for (int i = 0; i < Pages::totalpages; i++)
		{
			temp1 = pages[i]->pageid_getter();
			if (Helper::CompareString(temp1, page_id))
			{
				return pages[i];
			}
		}
	...
	{
		cout << "----------------------------------------------------------------------------------------------------------\n----------------------------------------------------------------------------------------------------------\n-----------------------------------------------FACEBOOK---------------------------------------------------\n----------------------------------------------------------------------------------------------------------\n----------------------------------------------------------------------------------------------------------\n";
		cout << "Enter a date in format of -day-month-year- to set current date;" << endl;
		Date::Present_Setter(14, 11, 2017);
		cout << "Todays date is set to: ";
		Date::present.printdate();
		cout << endl;
